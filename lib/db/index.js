...
//Simple async CRUD DB interface
...
async function test() {
  await save('testCollection', 'location3', {some: "punk", somethingElse: 'tramtadam'});
  let result = await getAll('testCollection');
  console.log("Result: " + JSON.stringify(result));

  const loc = await remove('testCollection', 'location3');

  result = await getAll('testCollection');
  console.log("Result: " + JSON.stringify(result));
}
...
export async function demoPopulate(){
...
export async function demoRead(){
  console.log(JSON.stringify(await getAll(constants.DB_COLLECTION_NAME_USERS)));
}
