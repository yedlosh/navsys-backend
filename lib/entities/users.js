...
export function getNextLocationId(user, distance = 1) {
  return user.path[user.path.indexOf(user.checkpointId) + distance];
}

export function getPreviousLocationId(user, distance = 1) {
  return user.path[user.path.indexOf(user.checkpointId) - distance];
}
