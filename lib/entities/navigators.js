...
export async function navigate({navigator, user, from, to, alert}) {
  let result;
  try {
    console.log(`Activating navigator:  ${navigator.id} userId: ${user.id} color: ${users.getHexColor(user)} from: ${from} - ${navigator.strip[from]} to ${to} - ${navigator.strip[to]}, alert: ${alert}`);
...
export function stopNavigation(navigator, userId) {
  ...
  axios.post("http://" + navigator.ip + ":5000" + '/stop/' + userId)
    .then( response => {
      console.log("Response from " + navigator.id + ": " + JSON.stringify(response.data));
      if(response.success){
...
export function audioCue(navigator, cue) {
  console.log("Playing audio cue | navigator: " + navigator.id + " cue: " + cue);
  axios.post("http://" + navigator.ip + ":5000" + '/play/' + cue)
    .then( response => {
      console.log("Response from " + navigator.id + ": " + JSON.stringify(response.data));
    })
    .catch( error => {
      console.log("Navigator audioCue error: " + error);
    });
}
