...
export async function handleTrack(trackRequest, findLocation) {
  ...
  if (!user) {
    ...
    if (location.id !== user.locationId){
      console.log("User " + userId + " reported location changed to:" + location.name);
      ...
      if (lastLocationNeighbors.includes(location.id)) {
        if(users.verifyLocationStability(user, location.id, 1)){
          console.log("User " + userId + " did change location to:" + location.name);

          if (location.id === users.getNextPathLocationId(user)) {
            ...
            if(location.id === user.destinationId){
              ...
              if(location.id === users.getNextCheckpoint(user)) {
...
export async function lightUpNextCheckpoint(user) {
  ...
  if(users.getNextCheckpoint(user, distance)) {
    const nextCheckpoint = await locations.getLocation(users.getNextCheckpoint(user, distance));
