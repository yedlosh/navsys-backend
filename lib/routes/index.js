...
export async function register(req, res, next) {
  ...
  try {
    ...
    return res.status(500).json({ error: 'Failed to process register request: ' + error});
  }
}
...
export async function getDestinations(req, res, next) {
  try {
    ...
    return res.status(500).json({ error: 'Could not get destinations', message: error.message});
  }
}
